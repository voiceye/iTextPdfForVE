...
import com.sga.text.pdf.internal.PolylineShape;
...
*/
    ...
    private boolean drawImage(Image img, Image mask, AffineTransform xform, Color bgColor, ImageObserver obs) {
        ...
        try {
            com.sga.text.Image image = null;
            if(!convertImagesToJPEG){
                image = com.sga.text.Image.getInstance(img, bgColor);
            }
            else{
                ...
                scaled.flush();
                scaled = null;
                image = com.sga.text.Image.getInstance(baos.toByteArray());

            }
            if (mask!=null) {
                com.sga.text.Image msk = com.sga.text.Image.getInstance(mask, null, true);
                msk.makeMask();
    ...
    private void setPaint(boolean invert, double xoffset, double yoffset, boolean fill) {
        if (paint instanceof Color) {
            ...
            try {
                ...
                com.sga.text.Image image = com.sga.text.Image.getInstance(img, null);
