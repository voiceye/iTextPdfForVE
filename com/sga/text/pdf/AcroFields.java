...
import com.sga.text.pdf.PRTokeniser.TokenType;
import com.sga.text.pdf.codec.Base64;
...
public class AcroFields {
	...
	// pkh
	public PdfString my_m;
	public PdfString my_name;
	public PdfString my_reason;
	// lkd
	public PdfName my_filter;
	public PdfString my_contents;
	public byte[] my_data;
	/**
	...
	/**
	 * Gets the list of appearance names. Use it to get the names allowed with radio
	 * and checkbox fields. If the /Opt key exists the values will also be included.
	 * The name 'Off' may also be valid even if not returned in the list.
	 *
	 ...
	 * the field doesn't exist or the field type is not list or combo it will return
	 * <CODE>null</CODE>.
	 *
	...
	/**
	 * Gets the list of display option values from fields of type list or combo. If
	 * the field doesn't exist or the field type is not list or combo it will return
	 * <CODE>null</CODE>.
	 *
	 * @param fieldName the field name
	 ...
	 * not set the value or appearance. For that, calling <CODE>setField()</CODE> is
	 * required.
	 * <p>
	 * An example:
	 * <p>
	...
	PdfAppearance getAppearance(PdfDictionary merged, String values[], String fieldName)
		...
		if (!PdfName.CH.equals(fieldType))
			throw new DocumentException(MessageLocalization
					.getComposedMessage("an.appearance.was.requested.without.a.variable.text.field"));
	 ...
	 * <CODE>BaseColor</CODE>. If <code>null</code> removes the background.<br>
	 * <li>bordercolor - sets the border color. The value for this entry is a
	 * <CODE>BaseColor</CODE>. If <code>null</code> removes the border.<br>
	 * </ul>
	 *
	 ...
	 * <p>
	 * <ul>
	 * <li>flags - a set of flags specifying various characteristics of the field's
	 * widget annotation. The value of this entry replaces that of the F entry in
	 * the form's corresponding annotation dictionary.<br>
	 * <li>setflags - a set of flags to be set (turned on) in the F entry of the
	 * form's corresponding widget annotation dictionary. Bits equal to 1 cause the
	 * corresponding bits in F to be set to 1.<br>
	 * <li>clrflags - a set of flags to be cleared (turned off) in the F entry of
	 * the form's corresponding widget annotation dictionary. Bits equal to 1 cause
	 * the corresponding bits in F to be set to 0.<br>
	 * <li>fflags - a set of flags specifying various characteristics of the field.
	 * The value of this entry replaces that of the Ff entry in the form's
	 ...
	 * <li>clrfflags - a set of flags to be cleared (turned off) in the Ff entry of
	 ...
	 * @throws com.sga.text.DocumentException o error
	 */
	 ...
	 * "bgcolor", BaseColor.BLUE, null); This won't have any effect, unless you use
	 * regenerateField("f") after changing the property.
	 *
	 * @param name the fully qualified field name or the partial name in the case of
	 *             XFA forms
	 * @throws IOException       on error
	 * @throws DocumentException on error
	...
	/**
	 * Sets the rich value for the given field. See <a href=
	 * "http://www.adobe.com/content/dam/Adobe/en/devnet/pdf/pdfs/PDF32000_2008.pdf">PDF
	 * Reference</a> chapter 12.7.3.4 (Rich Text) and 12.7.4.3 (Text Fields) for
	 * further details.
	 * 
	 ...
	 * build the appearance in the cases where the value is modified by Acrobat with
	 * JavaScript and the algorithm is known.
	 *
	 * @param name    the fully qualified field name or the partial name in the case
	 *                of XFA forms
	 * @param value   the field value
		...
		for (int k = 0; k < item.size(); ++k) {
			try {
				...
				if (rotation != 0) {
					...
					switch (rotation) {
					case 270:
						r = new Rectangle(pageSize.getTop() - r.getBottom(), r.getLeft(),
								pageSize.getTop() - r.getTop(), r.getRight());
						break;
					case 180:
						...
						break;
					case 90:
						r = new Rectangle(r.getBottom(), pageSize.getRight() - r.getLeft(), r.getTop(),
	 ...
	 *
	 * @param name the field name
	 * @param page the page to remove the field from or -1 to remove it from all the
	 *             pages
	 ...
	 * speed-up form filling but the results can be unexpected in Acrobat. Don't use
	 * it unless your environment is well controlled. The default is
	 * <CODE>true</CODE>.
	 *
	...
	public static class Item {
		...
		/**
		 * This function writes the given key/value pair to all the instances of merged,
		 * widget, and/or value, depending on the <code>writeFlags</code> setting
		 *
		 * @since 2.1.5
		 *
		...
		/**
		 * An array of <CODE>Integer</CODE> with the page numbers where the widgets are
		 * displayed.
		 *
		 * @since 5.0.2 public is now protected
		 */
		...
		/**
		 * Retrieve the merged dictionary for the given instance. The merged dictionary
		 * contains all the keys present in parent fields, though they may have been
		 * overwritten (or modified?) by children. Example: a merged radio field dict
		 * will contain /V
		 *
		 * @since 2.1.5
		 * @param idx instance index
	 ...
	 * 
	 * @param name the field name
	 * @return true if the field was signed, false if the field was not signed or
	 *         not found
	 * @since 5.0.5
	 */
	...
	public ArrayList<String> getSignatureNames() {
		...
		sigNames = new LinkedHashMap<String, int[]>(); // @chyoon (2022.12.15) 다중 전자서명 검증 시 순서대로 검증하기 위해 LinkedHashMap 사용
	...
	public PdfPKCS7 verifySignature(String name) {
		return verifySignature(name, "SGAProvider");
	}
	 ...
	 * 	Certificate pkc[] = pk.getCertificates();
	 * 	System.out.println("Subject: " + PdfPKCS7.getSubjectFields(pk.getSigningCertificate()));
	 * 	System.out.println("Document mod
	 * ified: " + !pk.verify());
	 * 	Object fails[] = PdfPKCS7.verifyCertificates(pkc, kall, null, cal);
	 * 	if (fails == null)
	 * 		System.out.println("Certificates verified against the KeyStore");
	...
	private void myByteRange(PdfDictionary v) {
		...
		try {
			...
			/* @chyoon 2022.10.21 : ByteRange에 해당하는 데이터만 추출하도록 수정
			* ex) 기존 로직에서는 전자서명된 문서의 맨 마지막에 데이터를 추가한 경우 '변조'로 인식. (원문 데이터 영역을 ByteRange에 명시된 대로 추출하지 않았음)  
			*/
			int start, length, next; 
			for (int k = 0; k < b.size(); ++k) {
				int nCnt = 0;
				start = b.getAsNumber(k).intValue();
				length = b.getAsNumber(++k).intValue();
				rf.seek(start);
				next = rf.read();
				while (next > -1) {
					bos.write(next);
					next = rf.read();
					nCnt++;					
					if ((k == 1 || k == 3) && (length <= nCnt))
						break;
				}
			}
			bos.flush();
			my_data = bos.toByteArray();
		} catch (Exception e) {
			throw new ExceptionConverter(e);
		} finally {
			try {
	...
	public PdfPKCS7 verifySignature(String name, String provider) {
		...
		try {
			...
			PdfString contents = v.getAsString(PdfName.CONTENTS);
			// pkh
			my_name = v.getAsString(PdfName.NAME);
			my_m = v.getAsString(PdfName.M);
			my_reason = v.getAsString(PdfName.REASON);
			// lkd
			my_filter = v.getAsName(PdfName.FILTER);
			if (my_filter.equals(PdfName.SGA_TCBL)) {
				myByteRange(v);
				my_contents = contents;
				return null;
			} else if (my_filter.equals(PdfName.SGA_GPKI)) {
				myByteRange(v);
				my_contents = contents;
				return null;
			} else if (my_filter.equals(PdfName.ADOBE_PPKLITE)) {
				myByteRange(v);
				my_contents = contents;
				return null;
			} else if (my_filter.equals(PdfName.ePapyrus) || (my_filter.equals(PdfName.DreamSecurity))) {
				myByteRange(v);
				my_contents = contents;
				return null;
			} else {
	...
	/**
	 * Sets a cache for field appearances. Parsing the existing PDF to create a new
	 * TextField is time expensive. For those tasks that repeatedly fill the same
	 * PDF with different field values the use of the cache has dramatic speed
	...
	/**
	 * Adds a substitution font to the list. The fonts in this list will be used if
	 * the original font doesn't contain the needed glyphs.
	 *
	 * @param font the font
	 */
	 ...
	 * can be changed and be used to replace an existing one, with the same name or
	 * other name, as long is it is in the same document. To replace an existing
	 * pushbutton call {@link #replacePushbuttonField(String,PdfFormField)}.
	 *
	...
	/**
	 * Replaces the first field with a new pushbutton. The pushbutton can be created
	 * with {@link #getNewPushbuttonFromField(String)} from the same document or it
	 * can be a generic PdfFormField of the type pushbutton.
	 *
	 * @param field  the field name
