...
public class PdfPKCS7 {
	...
	static {
		...
		algorithmNames.put("1.2.410.200004.1.21", "KCDSA1");
		algorithmNames.put("1.2.410.200004.1.38", "KCDSA");
	...
	public PdfPKCS7(byte[] contentsKey, byte[] certsKey, String provider) {
		try {
			...
			X509StreamParserSpi spi = null;
			spi.engineInit(new ByteArrayInputStream(certsKey));
			certs = spi.engineReadAll();
			signCerts = certs;
	...
	/*
	 * private void findCRL(ASN1Sequence seq) throws IOException,
	 * CertificateException, CRLException { try { crls = new ArrayList<CRL>(); for
	 * (int k = 0; k < seq.size(); ++k) { ByteArrayInputStream ar = new
	 * ByteArrayInputStream(seq.getObjectAt(k).getASN1Object().getDEREncoded());
	 * CertificateFactory cf = CertificateFactory.getInstance("X.509"); X509CRL crl
	 * = (X509CRL)cf.generateCRL(ar); crls.add(crl); } } catch (Exception ex) { //
	 * ignore } }
	 */
	...
	/*
	 * private void findOcsp(ASN1Sequence seq) throws IOException { basicResp =
	 * null; boolean ret = false; while (true) { if (seq.getObjectAt(0) instanceof
	 * ASN1ObjectIdentifier &&
	 * ((ASN1ObjectIdentifier)seq.getObjectAt(0)).getId().equals(
	 * OCSPObjectIdentifiers.id_pkix_ocsp_basic.getId())) { break; } ret = true; for
	 * (int k = 0; k < seq.size(); ++k) { if (seq.getObjectAt(k) instanceof
	 * ASN1Sequence) { seq = (ASN1Sequence)seq.getObjectAt(0); ret = false; break; }
	 * if (seq.getObjectAt(k) instanceof ASN1TaggedObject) { ASN1TaggedObject tag =
	 * (ASN1TaggedObject)seq.getObjectAt(k); if (tag.getObject() instanceof
	 * ASN1Sequence) { seq = (ASN1Sequence)tag.getObject(); ret = false; break; }
	 * else return; } } if (ret) return; } DEROctetString os =
	 * (DEROctetString)seq.getObjectAt(1); ASN1InputStream inp = new
	 * ASN1InputStream(os.getOctets()); BasicOCSPResponse resp =
	 * BasicOCSPResponse.getInstance(inp.readObject()); basicResp = new
	 * BasicOCSPResp(resp); }
	 */
	/**
		...
		try {
			...
			try {
				...
				throw new IllegalArgumentException(
						MessageLocalization.getComposedMessage("can.t.decode.pkcs7signeddata.object", new Object[0]));
			}

			if (!(pkcs instanceof ASN1Sequence)) {
				throw new IllegalArgumentException(MessageLocalization
						.getComposedMessage("not.a.valid.pkcs.7.object.not.a.sequence", new Object[0]));
			} else {
				ASN1Sequence signedData = (ASN1Sequence) pkcs;
				ASN1ObjectIdentifier objId = (ASN1ObjectIdentifier) signedData.getObjectAt(0);
				if (!objId.getId().equals("1.2.840.113549.1.7.2")) {
					throw new IllegalArgumentException(MessageLocalization
							.getComposedMessage("not.a.valid.pkcs.7.object.not.signed.data", new Object[0]));
				} else {
					ASN1Sequence content = (DLSequence)((DLTaggedObject) signedData.getObjectAt(1)).getObject();
					...
					/*
					 * while (e.hasMoreElements()) { ASN1Sequence s = (ASN1Sequence)
					 * e.nextElement(); ASN1ObjectIdentifier o = (ASN1ObjectIdentifier)
					 * s.getObjectAt(0); this.digestalgos.add(o.getId()); }
					 */
					...
					this.certs = (Collection<Certificate>) cf.generateCertificates(new ByteArrayInputStream(contentsKey));
					...
					if (signerInfos.size() != 1) {
						...
						//Iterator var18 = this.certs.iterator();
						//Iterator var18 = certs.iterator();
						X509Name issuer = null;
						
						while (certs.iterator().hasNext()) {
							//ASN1Sequence asn1Sequence = (ASN1Sequence) cert.nextElement();
							Object element = certs.iterator().next();
							X509Certificate certificate = (X509Certificate) element;
							issuer = new X509Name(certificate.getSubjectDN().toString());
						...
						if (this.signCert == null) {
							throw new IllegalArgumentException(MessageLocalization.getComposedMessage(
									...
									new Object[] { issuer.toString()/*getName()*/ + " / " + serialNumber.toString(16) }));
						} else {
							this.signCertificateChain();
							this.digestAlgorithm = ((ASN1ObjectIdentifier) ((ASN1Sequence) signerInfo.getObjectAt(2))
									.getObjectAt(0)).getId();
							next = 3;
							ASN1Set setout;
							ASN1Sequence seqout;
							...
							if (signerInfo.getObjectAt(next) instanceof ASN1TaggedObject) {
								...
								sseq = ASN1Set.getInstance(tagsig, false);
								this.sigAttr = sseq.getEncoded("DER");

								for (int k = 0; k < sseq.size(); ++k) {
									ASN1Sequence seq2 = (ASN1Sequence) sseq.getObjectAt(k);
									if (((ASN1ObjectIdentifier) seq2.getObjectAt(0)).getId()
											.equals("1.2.840.113549.1.9.4")) {
										setout = (ASN1Set) seq2.getObjectAt(1);
										this.digestAttr = ((DEROctetString) setout.getObjectAt(0)).getOctets();
									} else if (((ASN1ObjectIdentifier) seq2.getObjectAt(0)).getId()
											.equals("1.2.840.113583.1.1.8")) {
										setout = (ASN1Set) seq2.getObjectAt(1);
								...
								if (this.digestAttr == null) {
									throw new IllegalArgumentException(MessageLocalization.getComposedMessage(
											"authenticated.attribute.is.missing.the.digest", new Object[0]));
								}

								++next;
							}

							this.digestEncryptionAlgorithm = ((ASN1ObjectIdentifier) ((ASN1Sequence) signerInfo
							...
							if (next < signerInfo.size() && signerInfo.getObjectAt(next) instanceof DERTaggedObject) {
								...
								sseq = ASN1Set.getInstance(taggedObject, false);
								AttributeTable attble = new AttributeTable(sseq);
								Attribute ts = attble.get(PKCSObjectIdentifiers.id_aa_signatureTimeStampToken);
								if (ts != null && ts.getAttrValues().size() > 0) {
									setout = ts.getAttrValues();
									seqout = ASN1Sequence.getInstance(setout.getObjectAt(0));
									ContentInfo contentInfo = new ContentInfo(seqout);
							...
							if (this.sig.getAlgorithm().equals("SHA256withKCDSA1") || this.sig.getAlgorithm().equals("SHA256withKCDSA")) {
	...
	/*
	 * @SuppressWarnings("unchecked") public PdfPKCS7(byte[] contentsKey, String
	 * provider) { try { this.provider = provider; ASN1InputStream din = new
	 * ASN1InputStream(new ByteArrayInputStream(contentsKey));
	 * 
	 * // // Basic checks to make sure it's a PKCS#7 SignedData Object //
	 * //ASN1Object pkcs;
	 * 
	 * //PdfPKCS7 pkcs; ASN1Primitive pkcs;
	 * 
	 * try { pkcs = din.readObject(); } catch (IOException e) { throw new
	 * IllegalArgumentException(MessageLocalization.getComposedMessage(
	 * "can.t.decode.pkcs7signeddata.object")); } if (!(pkcs instanceof
	 * ASN1Sequence)) { throw new
	 * IllegalArgumentException(MessageLocalization.getComposedMessage(
	 * "not.a.valid.pkcs.7.object.not.a.sequence")); } ASN1Sequence signedData =
	 * (ASN1Sequence)pkcs; ASN1ObjectIdentifier objId =
	 * (ASN1ObjectIdentifier)signedData.getObjectAt(0); //ASN1ObjectIdentifier objId
	 * = (ASN1ObjectIdentifier)signedData.getObjectAt(0); if
	 * (!objId.getId().equals(ID_PKCS7_SIGNED_DATA)) throw new
	 * IllegalArgumentException(MessageLocalization.getComposedMessage(
	 * "not.a.valid.pkcs.7.object.not.signed.data")); ASN1Sequence content =
	 * (ASN1Sequence)((DERTaggedObject)signedData.getObjectAt(1)).getObject(); //
	 * the positions that we care are: // 0 - version // 1 - digestAlgorithms // 2 -
	 * possible ID_PKCS7_DATA // (the certificates and crls are taken out by other
	 * means) // last - signerInfos
	 * 
	 * // the version version =
	 * ((DERTaggedObject)content.getObjectAt(0)).getTagNo();
	 * 
	 * // the digestAlgorithms digestalgos = new HashSet<String>();
	 * Enumeration<ASN1Sequence> e = ((ASN1Set)content.getObjectAt(1)).getObjects();
	 * while (e.hasMoreElements()) { ASN1Sequence s = e.nextElement();
	 * ASN1ObjectIdentifier o = (ASN1ObjectIdentifier)s.getObjectAt(0);
	 * digestalgos.add(o.getId()); }
	 * 
	 * // the certificates X509StreamParserSpi cr = null; cr.engineInit(new
	 * ByteArrayInputStream(contentsKey)); certs = cr.engineReadAll();
	 * 
	 * // the possible ID_PKCS7_DATA ASN1Sequence rsaData =
	 * (ASN1Sequence)content.getObjectAt(2); if (rsaData.size() > 1) {
	 * DEROctetString rsaDataContent =
	 * (DEROctetString)((DERTaggedObject)rsaData.getObjectAt(1)).getObject();
	 * RSAdata = rsaDataContent.getOctets(); }
	 * 
	 * // the signerInfos int next = 3; while (content.getObjectAt(next) instanceof
	 * DERTaggedObject) ++next; ASN1Set signerInfos =
	 * (ASN1Set)content.getObjectAt(next); if (signerInfos.size() != 1) throw new
	 * IllegalArgumentException(MessageLocalization.getComposedMessage(
	 * "this.pkcs.7.object.has.multiple.signerinfos.only.one.is.supported.at.this.time"
	 * )); ASN1Sequence signerInfo = (ASN1Sequence)signerInfos.getObjectAt(0); //
	 * the positions that we care are // 0 - version // 1 - the signing certificate
	 * issuer and serial number // 2 - the digest algorithm // 3 or 4 -
	 * digestEncryptionAlgorithm // 4 or 5 - encryptedDigest signerversion =
	 * ((ASN1Integer)signerInfo.getObjectAt(0)).getValue().intValue(); // Get the
	 * signing certificate ASN1Sequence issuerAndSerialNumber =
	 * (ASN1Sequence)signerInfo.getObjectAt(1); X509Principal issuer = new
	 * X509Principal(issuerAndSerialNumber.getObjectAt(0).getASN1Object().getEncoded(
	 * )); BigInteger serialNumber =
	 * ((ASN1Integer)issuerAndSerialNumber.getObjectAt(1)).getValue(); for (Object
	 * element : certs) { X509Certificate cert = (X509Certificate)element; if
	 * (issuer.equals(cert.getIssuerDN()) &&
	 * serialNumber.equals(cert.getSerialNumber())) { signCert = cert; break; } } if
	 * (signCert == null) { throw new
	 * IllegalArgumentException(MessageLocalization.getComposedMessage(
	 * "can.t.find.signing.certificate.with.serial.1", issuer.getName() + " / " +
	 * serialNumber.toString(16))); } signCertificateChain(); digestAlgorithm =
	 * ((ASN1ObjectIdentifier)((ASN1Sequence)signerInfo.getObjectAt(2)).getObjectAt(0
	 * )).getId(); next = 3; if (signerInfo.getObjectAt(next) instanceof
	 * ASN1TaggedObject) { ASN1TaggedObject tagsig =
	 * (ASN1TaggedObject)signerInfo.getObjectAt(next); ASN1Set sseq =
	 * ASN1Set.getInstance(tagsig, false); sigAttr =
	 * sseq.getEncoded(ASN1Encodable.DER);
	 * 
	 * for (int k = 0; k < sseq.size(); ++k) { ASN1Sequence seq2 =
	 * (ASN1Sequence)sseq.getObjectAt(k); if
	 * (((ASN1ObjectIdentifier)seq2.getObjectAt(0)).getId().equals(ID_MESSAGE_DIGEST)
	 * ) { ASN1Set set = (ASN1Set)seq2.getObjectAt(1); digestAttr =
	 * ((DEROctetString)set.getObjectAt(0)).getOctets(); } else if
	 * (((ASN1ObjectIdentifier)seq2.getObjectAt(0)).getId().equals(ID_ADBE_REVOCATION
	 * )) { ASN1Set setout = (ASN1Set)seq2.getObjectAt(1); ASN1Sequence seqout =
	 * (ASN1Sequence)setout.getObjectAt(0); for (int j = 0; j < seqout.size(); ++j)
	 * { ASN1TaggedObject tg = (ASN1TaggedObject)seqout.getObjectAt(j); if
	 * (tg.getTagNo() == 0) { ASN1Sequence seqin = (ASN1Sequence)tg.getObject();
	 * findCRL(seqin); } if (tg.getTagNo() == 1) { ASN1Sequence seqin =
	 * (ASN1Sequence)tg.getObject(); findOcsp(seqin); } } } } if (digestAttr ==
	 * null) throw new
	 * IllegalArgumentException(MessageLocalization.getComposedMessage(
	 * "authenticated.attribute.is.missing.the.digest")); ++next; }
	 * digestEncryptionAlgorithm =
	 * ((ASN1ObjectIdentifier)((ASN1Sequence)signerInfo.getObjectAt(next++)).
	 * getObjectAt(0)).getId(); digest =
	 * ((DEROctetString)signerInfo.getObjectAt(next++)).getOctets(); if (next <
	 * signerInfo.size() && signerInfo.getObjectAt(next) instanceof DERTaggedObject)
	 * { DERTaggedObject taggedObject = (DERTaggedObject)
	 * signerInfo.getObjectAt(next); ASN1Set unat =
	 * ASN1Set.getInstance(taggedObject, false); AttributeTable attble = new
	 * AttributeTable(unat); Attribute ts =
	 * attble.get(PKCSObjectIdentifiers.id_aa_signatureTimeStampToken); if (ts !=
	 * null && ts.getAttrValues().size() > 0) { ASN1Set attributeValues =
	 * ts.getAttrValues(); ASN1Sequence tokenSequence =
	 * ASN1Sequence.getInstance(attributeValues.getObjectAt(0)); ContentInfo
	 * contentInfo = new ContentInfo(tokenSequence); this.timeStampToken = new
	 * TimeStampToken(contentInfo); } } if (RSAdata != null || digestAttr != null) {
	 * if (provider == null || provider.startsWith("SunPKCS11")) messageDigest =
	 * MessageDigest.getInstance(getHashAlgorithm()); else messageDigest =
	 * MessageDigest.getInstance(getHashAlgorithm(), provider); } if (provider ==
	 * null) sig = Signature.getInstance(getDigestAlgorithm()); else sig =
	 * Signature.getInstance(getDigestAlgorithm(), provider);
	 * sig.initVerify(signCert.getPublicKey()); } catch (Exception e) { throw new
	 ...
	 */
	/*
	 * public boolean verify() throws SignatureException { if (verified) return
	 * verifyResult; if (sigAttr != null) { final byte [] msgDigestBytes =
	 * messageDigest.digest(); boolean verifyRSAdata = true; sig.update(sigAttr); if
	 * (RSAdata != null) verifyRSAdata = Arrays.equals(msgDigestBytes, RSAdata);
	 * verifyResult = Arrays.equals(msgDigestBytes, digestAttr) &&
	 * sig.verify(digest) && verifyRSAdata; } else { if (RSAdata != null)
	 * sig.update(messageDigest.digest()); verifyResult = sig.verify(digest); }
	 * verified = true; return verifyResult; }
	 */
	...
	/**
	 * Get the X.509 sign certificate chain associated with this PKCS#7 object. Only
	 * the certificates used for the main signature will be returned, with the
	...
	public String getDigestAlgorithm() {
		// kcdsa1 추가 ==> sha256withkcdsa1 -> SGAProvider oid Name ==> KCDSAwithSHA256 [2022-06-02] 박건헌
	 ...
	 */
	public static Object[] verifyCertificates(Certificate certs[], KeyStore keystore, Collection<CRL> crls,
			Calendar calendar) {
		if (calendar == null)
			calendar = new GregorianCalendar();
...
*/
			...
			for (int i = 0; i < AccessDescriptions.size(); i++) {
				...
				if (AccessDescription.size() != 2) {
					...
					if (AccessDescription.getObjectAt(0) instanceof ASN1ObjectIdentifier
							&& ((ASN1ObjectIdentifier) AccessDescription.getObjectAt(0)).getId()
	 ...
	 * @return a ASN1Object
	 */
	private static ASN1Object /*ASN1Object*/ getIssuer(byte[] enc) {
		try {
	 ...
	 * @param RSAdata                   the extra data that goes into the data tag
	 *                                  in PKCS#7
	 * @param digestEncryptionAlgorithm the encryption algorithm. It may must be
	 *                                  <CODE>null</CODE> if the <CODE>digest</CODE>
	 *                                  is also <CODE>null</CODE>. If the
	 *                                  <CODE>digest</CODE> is not <CODE>null</CODE>
	...
	public byte[] getEncodedPKCS7(byte secondDigest[], Calendar signingTime, TSAClient tsaClient, byte[] ocsp) {
		try {
			...
			if (secondDigest != null && signingTime != null) {
				signerinfo.add(
						new DERTaggedObject(false, 0, getAuthenticatedAttributeSet(secondDigest, signingTime, ocsp)));
			}
	 ...
	 * start with the timeStampToken (signedData 1.2.840.113549.1.7.2). Token is the
	 * TSA response without response status, which is usually handled by the (vendor
	 * supplied) TSA request/response interface).
	 * 
	 ...
	 * The document digest is generated and put inside the attribute. The signing is
	 * done over the DER encoded authenticatedAttributes. This method provides that
	 * encoding and the parameters must be exactly the same as in
	 ...
	 * @return the byte array representation of the authenticatedAttributes ready to
	 *         be signed
	 */
		...
		public X509Name(ASN1Sequence seq) {
			...
			while (e.hasMoreElements()) {
				...
				for (int i = 0; i < set.size(); i++) {
					...
					vs.add(((ASN1OctetString) s.getObjectAt(1)).toString());
				}
			}
		}
