...
public class PdfSignatureAppearance {
    ...
    private String sgaReserve;
    private String cn; //lkd
    private String timestamp_date; //lkd
    private String timestamp_time; //lkd
    private String timestamp_site; //lkd
    
    private String blockchain_name;
    private String blockchain_date;  //lkd
    private String blockchain_time; //lkd
    private String blockchain_site; //lkd
    ...
    */
    ...
    public void setSignatureType(int tsType) {
        this.tsType = tsType;
    }
    ...
    public PdfTemplate getAppearance() throws DocumentException {
        ...
        if (app[2] == null) {
            ...
            if(renderingMode == RenderingMode.TIMESTAMP)
            {
            	StringBuffer buf = new StringBuffer();
            	if(tsType == 0)
            	{
	            	buf.append(timestamp_date).append('\n');
	            	buf.append(timestamp_time).append('\n');
	            	buf.append(timestamp_site);
            	}
            	else
            	{
            		buf.append(timestamp_date).append(' ');
            ...
            else if(renderingMode == RenderingMode.BLOCKCHAIN)
            {
            	StringBuffer buf = new StringBuffer();
            	
            	buf.append(blockchain_date).append('\n');
            	buf.append(blockchain_time).append('\n');
            	buf.append(blockchain_site);

                text = buf.toString();
            }
            else if(renderingMode == RenderingMode.SGASTAMP)
            {
            	StringBuffer buf = new StringBuffer();
            ...
            else
	        {
            	if (layer2Text == null) {
	                StringBuffer buf = new StringBuffer();
	                buf.append("Digitally signed by ").append(cn).append('\n');
            ...
            Font font;
            if (layer2Font == null)
                font = new Font(Font.FontFamily.COURIER);  //font = new Font(); //
            else
            ...
            if (renderingMode == RenderingMode.NAME_AND_DESCRIPTION ||
            ...
            else if (renderingMode == RenderingMode.TIMESTAMP) {
                if (signatureGraphic == null) {
                    throw new IllegalStateException(MessageLocalization.getComposedMessage("a.signature.image.should.be.present.when.rendering.mode.is.graphic.only"));
                }
                if(tsType == 0)
                ...
                else
                {
                    ...
                    dataRect = new Rectangle(
                    		2,
                            2,
                            rect.getWidth() - 2, // take all space available
                            (rect.getHeight()-2-2));
                }
            ...
            case TIMESTAMP:
            	...
            	else
            	{
                    ...
                    im = Image.getInstance(signatureGraphic);
                    //im.scaleToFit(signatureRect.getWidth(), signatureRect.getHeight());
                    im.scaleAbsolute(signatureRect.getWidth(), signatureRect.getHeight());
            ...
            if(renderingMode != RenderingMode.GRAPHIC && renderingMode != RenderingMode.TIMESTAMP && renderingMode != RenderingMode.BLOCKCHAIN) {
            	if (size <= 0) {
            ...
            if(renderingMode == RenderingMode.TIMESTAMP) {
            	if(tsType == 0)
            	{
	            	if (size <= 0) {
	                    Rectangle sr = new Rectangle(dataRect.getWidth(), dataRect.getHeight());
	                    size = fitText(font, text, sr, 7, runDirection);
	                }
	                ColumnText ct = new ColumnText(t);
	                ct.setRunDirection(runDirection);
	                ct.setSimpleColumn(new Phrase(text, font), dataRect.getLeft(), dataRect.getBottom(), dataRect.getRight(), dataRect.getTop(), size, Element.ALIGN_CENTER);
	                ct.go();
            	}
            	else
            	{
            ...
            //lkd
            if(renderingMode == RenderingMode.BLOCKCHAIN) {
    ...
    public String getSGAReserve() {
        return this.sgaReserve;
    }
     ...
     * @param reason the signing reason
     */
    public void setSGAReserve(String sgaReserve) {
        this.sgaReserve = sgaReserve;
    }
    ...
    public void setTimestampDate(String timestamp_date) {
    ...
    public void setTimestampTime(String timestamp_time) {
        this.timestamp_time = timestamp_time;
    }
    
    public String getTimestampSite() {
        return this.timestamp_site;
    }

    public void setTimestampSite(String timestamp_site) {
        this.timestamp_site = timestamp_site;
    }
    ...
    public void setBlockchainName(String blockchain_name) {
        this.blockchain_name = blockchain_name;
    }
    
    public String getBlockchainDate() {
        return this.blockchain_date;
    }

    public void setBlockchainDate(String blockchain_date) {
        this.blockchain_date = blockchain_date;
    }
    
    public String getBlockchainTime() {
        return this.blockchain_time;
    }

    public void setBlockchainTime(String blockchain_time) {
        this.blockchain_time = blockchain_time;
    }
    
    public String getBlockchainSite() {
        return this.blockchain_site;
    }

    public void setBlockchainSite(String blockchain_site) {
     ...
     */
    public com.sga.text.pdf.PdfName getFilter() {
        return filter;
    }
     ...
     */
    public com.sga.text.Rectangle getPageRect() {
        return pageRect;
    }
    ...
    com.sga.text.pdf.ByteBuffer getSigout() {
        return sigout;
    }

    void setSigout(com.sga.text.pdf.ByteBuffer sigout) {
        this.sigout = sigout;
    }
    ...
    public void preClose(HashMap<PdfName, Integer> exclusionSizes) throws IOException, DocumentException {
        ...
        if (cryptoDictionary == null) {
            ...
            //lkd
            if(getSGAReserve() != null)
            	sigStandard.setSGAReserve(getSGAReserve());
            if (getContact() != null)
     ...
     */
    public com.sga.text.pdf.PdfDictionary getCryptoDictionary() {
     ...
     */
    public void setCryptoDictionary(com.sga.text.pdf.PdfDictionary cryptoDictionary) {
     ...
     */
    public com.sga.text.pdf.PdfStamper getStamper() {
        return stamper;
    }

    void setStamper(com.sga.text.pdf.PdfStamper stamper) {
     ...
     */
    public com.sga.text.pdf.PdfSigGenericPKCS getSigStandard() {
        return sigStandard;
    }
